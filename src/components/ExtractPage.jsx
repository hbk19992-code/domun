...
// 🟢 퍼블릭 API용 안정적인 Gemini 모델 라인업
const GEMINI_MODELS = ['gemini-2.5-flash', 'gemini-2.5-flash-lite', 'gemini-2.0-flash']
// 🟢 Groq에서 공식 권장하는 현역 최신 Llama 3.1 양대 산맥 모델명
...
// 🟢 [완전 수정] Groq 엔진 전용 순수 REST API 호출 통신부 (413 Payload Error 원천 차단)
async function callGroq(apiKey, model, text, systemPrompt) {
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.1,
      response_format: { type: "json_object" } // 구조화된 JSON 데이터 출력 강제 보장 옵션
    })
  });
...
// 🟢 Gemini 엔진 전용 순수 구글 규격 REST API 호출 통신부
...
export default function ExtractPage({ cards, onImport }) {
  ...
  // 🟢 [분기 완벽화] 엔진 상태에 부합하도록 payload 및 호출 분기를 물리적으로 완전 분리
  const runExtraction = useCallback(async (geminiParts, label, sourceText = null) => {
    ...
    try {
      ...
      if (apiEngine === 'groq') {
        ...
        // 자동 분기일 땐 안전 분량 한도(12,000자 미만) 및 키 확보 유무 동시 스캔
        useGroq = !!(sourceText && sourceText.length < 12000 && groqKey);
      }

      if (useGroq) {
        try {
          // 🟢 Groq가 선호하는 완벽한 텍스트 전송 구조로 호출
          raw = await extractWithGroq(groqKey, sourceText, prompt, setProgress);
        } catch(groqErr) {
          if (apiEngine === 'groq') {
            if (groqErr.message === '__GROQ_TOO_LONG__') {
              throw new Error("⚠️ 텍스트 분량이 Groq 엔진의 단일 제한 범위를 초과했습니다 (413 Payload Too Large).\n분량을 줄이시거나 상단 엔진을 'Gemini'로 변경하여 다시 실행해 주세요.");
            }
            throw groqErr;
          }
          
          console.warn("Groq 오류로 인하여 안전지대인 Gemini로 하이브리드 자동 전환합니다.", groqErr);
          if (!geminiKey) throw new Error("분량이 무거워 Gemini 백업 가동이 필요하나, 등록된 Gemini API 키가 없습니다.");
          raw = await extractWithGemini(geminiKey, geminiParts, prompt, setProgress);
        }
      } else {
  ...
  const handleFile = useCallback(async (f) => {
    ...
    if (ext === 'txt') {
      ...
      geminiParts = [{ text: `다음 텍스트에서 카드를 빠짐없이 추출해주세요:\n\n${text}` }]
      sourceText = text;
    } else {
    ...
    await runExtraction(geminiParts, f.name, sourceText)
  }, [geminiKey, groqKey, runExtraction])

  const handleTextSubmit = useCallback(async () => {
    if ((!geminiKey && !groqKey) || !textInput.trim()) return
    const geminiParts = [{ text: `다음 텍스트에서 카드를 빠짐없이 추출해주세요:\n\n${textInput}` }]
    await runExtraction(geminiParts, '텍스트', textInput)
        ...
        <>
          ...
          {inputMode === 'file' ? (
            ...
            >
              ...
              <div style={{ color: '#94a3b8', fontSize: 14, lineHeight: 1.6, wordBreak: 'keep-all' }}>PDF, Word, TXT 파일을 드래그하거나<br /><span style={{ color: '#818cf8', fontWeight: 600 }}>클릭하여 탐색기 열기</span></div>
              <div style={{ color: '#475569', fontSize: 11, marginTop: 10, lineHeight: 1.6, wordBreak: 'keep-all' }}>단원 및 테마 단위 분할 업로드 권장 (최대 {MAX_FILE_MB}MB)</div>
            </div>
          ) : (

...
// 🟢 Groq 공식 문서 권장 최신 프로덕션 모델인 llama-3.1 계열로 업그레이드
...
// 🟢 [완전 수정] Groq 표준 규격에 맞게 body 데이터를 최적화하여 413 Payload 에러 원천 차단
  ...
  const runExtraction = useCallback(async (parts, label, sourceText = null) => {
    ...
    try {
      ...
      if (apiEngine === 'groq') {
        ...
        // 자동 분기 모드일 때 글자 제한을 안전하게 12,000자 미만으로 적용
        ...
        try {
          ...
          if (apiEngine === 'groq') {
            if (groqErr.message === '__GROQ_TOO_LONG__') {
              throw new Error("⚠️ 텍스트 분량이 Groq 엔진의 단일 제한 범위를 초과했습니다(413 Payload Too Large).\n분량을 분할하여 입력하시거나 상단 엔진을 'Gemini'로 변경하여 다시 실행해 주세요.");
            }
            throw groqErr;
          }
          
          console.warn("Groq 실패 또는 용량 초과, Gemini로 안전하게 전환합니다.", groqErr);
          if (!geminiKey) throw new Error("분량이 많아 Gemini로 전환해야 하지만, 등록된 Gemini API 키가 없습니다.");
